...
const Auction = () => {
  ...
  const orders = async () => {
    try {
      const allorders = await axios.get(
        "http://energy-app-7-env.eba-mr4i5q5z.ap-south-1.elasticbeanstalk.com/api/auct/auction"
      );
  ...
  const bid = async (bidAmount, orderId, bidderWallet, ownerwallet) => {
    if (ownerwallet === walletAdd) {
      ...
      try {
        const res = await axios.post("http://energy-app-7-env.eba-mr4i5q5z.ap-south-1.elasticbeanstalk.com/api/auct/bid", {
          bidAmount,
          orderId,
          bidderWallet,
        });
  ...
  const deleteitem = async (id) => {
    ...
    try {
      const dlt = await axios.delete(
        `http://energy-app-7-env.eba-mr4i5q5z.ap-south-1.elasticbeanstalk.com/api/auct/delete/${id}`,
